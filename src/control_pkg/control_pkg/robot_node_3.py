    ...
    def move_step_cb(self, req, res):
        try:
            if req.target_size == "YAW":
                ...
                # YAW + XY + Z 한 모션 (AMR load_node 방식).
                # move_l_rel(Tool)의 병진은 이동 시작(HOME) 프레임 기준으로 적용되고,
                # rz(yaw)는 tool Z축 방향 자체를 안 바꾸므로 HOME에서 측정한 dx/dy/z를
                # 한 번에 합쳐도 된다. req.z 는 master가 계산한 상대 하강량(z_move - Z_MARGIN).
                dx = -(req.x * 1000.0) + self.CAM_Y_OFF
                dy = (req.y * 1000.0) + self.CAM_X_OFF
                self.robot.move_l_rel(
                    self.rc,
                    np.array([dy, dx, req.z, 0, 0, req.yaw], dtype=float),
                    self.L_VEL, self.L_ACC, rb.ReferenceFrame.Tool
                )
                self.wait_move(f"APPROACH(yaw={req.yaw:.1f})")
